...
def get_entry_date():
    ...
    while flag:
        ...
        # line 31, moved the datetime.strptime up here and gave it a variable to store in
        # line 32, fixed the syntax error on user_data
        actual_date = datetime.strptime(user_date, "%d.%m.%Y.")

        try:
            # delete .date at the end of code
        ...
        # print(actual_date)
        #
        # # line 33-34, replaced a try statement with if
...
def check_age(num_junior: int):
    not_junior = 0

    for i in range(0, num_junior):
        flag = True

        while flag:
            # added [i] instead of (i)
